...
function authz(options = () => {}) {
    ...
    return (req, res, next) => {
        ...
        const resources = getResourcesStructure(req);
 ...
 * @return {string} The PDP endpoint that serves authz decisions
 */
function authzEndpoint(hostname, port, policyPath) {
    ...
    const hostport = hostname.concat(':'.concat(port.toString()));
    ...
    const endpoint = new URL(policyPath, hostport);
 ...
 * @param {object} req
 * @return {any} resources structure JSON
 */
function getResourcesStructure(req) {
    return {
